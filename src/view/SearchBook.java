...
public class SearchBook extends JPanel {
    ...
    private void init() {
    	...
    	searcher = new BookSearcher(new TitleSearch());
                    ...
                    if (searchText.trim().length() == 0) {
                        ...
                        switch (comboBox.getSelectedItem().toString()) {
                            case "제어번호":
                            	...
                            	searcher.setSearchStrategy(new IndexSearch());
                                model.setNumRows(0);
                                for(int i = 0; i < searcher.search(searchText).getDataVector().size();i++) {
                                	model.addRow(searcher.search(searchText).getDataVector().get(i));
                                }
                                break;
                            case "제목":
                            ...
                            case "저자":
                                ...
                                break;
                            case "발행처":
                            	searcher.setSearchStrategy(new PublisherSearch());
                            	model.setNumRows(0);
                                for(int i = 0; i < searcher.search(searchText).getDataVector().size();i++) {
                                	model.addRow(searcher.search(searchText).getDataVector().get(i));
                                }
                                break;
                            case "발행년도":
                            	searcher.setSearchStrategy(new YearSearch());
                            	model.setNumRows(0);
                                for(int i = 0; i < searcher.search(searchText).getDataVector().size();i++) {
                                	model.addRow(searcher.search(searchText).getDataVector().get(i));
                                }
